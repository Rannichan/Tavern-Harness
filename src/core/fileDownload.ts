...
async function pickSaveFileHandle(suggestedName: string): Promise<FileSystemFileHandle | null> {
 ...
 *  不可用时回退为浏览器默认下载目录。返回 true 表示已保存/已下载。 */
export async function saveTextFile(content: string, mime: string, suggestedName: string): Promise<boolean> {
...
export function saveJsonFile(payload: unknown, suggestedName: string): Promise<boolean> {
